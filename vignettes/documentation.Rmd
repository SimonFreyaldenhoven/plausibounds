...
The approach approximates $\beta$ by a *surrogate path* $\beta_M = P(M)\beta$, where $M$ indexes a model in a pre-specified universe $\mathcal{M}$, alluded to above. The model universe is specified a priori and includes polynomial specifications (up to cubic) and penalized smoothing that shrinks first (effect is "smooth") and third (effects "die off") differences. 
...
where the degrees of freedom is $\text{df}(M) = \mathrm{trace}(P(M))$. Therefore, the main algorithm of the package simply iterates over all models in the pre-specified universe to find $$\hat{M}$$. 
...
#>   .. ..$ d   : num [1:12, 1] 0.08 -0.125 -0.257 -0.329 -0.354 ...
#>   .. ..$ Vd  : num [1:12, 1:12] 0.01096 0.005009 0.001171 -0.000937 -0.001697 ...
#>   .. ..$ MD  : num [1, 1] 4.05
#>   .. ..$ J   : cplx [1:12, 1:12] 0.739+0i 0.552+0i 0.133+0i ...
#>   .. ..$ pval: num [1, 1] 0.853
...
#> Warning in data(estimates_pretrends): data set 'estimates_pretrends' not found
data(var_pretrends)
#> Warning in data(var_pretrends): data set 'var_pretrends' not found
...
#> Error: object 'estimates_pretrends' not found

print(pb_event)
#> Error: object 'pb_event' not found
```
